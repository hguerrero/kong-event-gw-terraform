...
2. Edit `client1-config.properties` and replace the placeholder values with the ones produced by Terraform:
   ...
   sasl.oauthbearer.token.endpoint.url=https://your-auth-server.konghq.tech/oauth2/token
...
- **Topic not found**:
  ...
  - When connecting, use topic names WITHOUT the `internal-` prefix
  - Example: Backend topic `internal-test-topic` is accessed as `test-topic`
